...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AdaptiveAggregationStagedRecordsMerged, "How many staged records the adaptive aggregation merged away as duplicate keys during conversion and coalescing.", ValueType::Number) \
    ...
    M(AdaptiveAggregationSealedChunks, "How many chunks the adaptive aggregation produced by coalescing buffered staging batches.", ValueType::Number) \
    M(AdaptiveAggregationStagedChunkSplits, "How many staged chunks the adaptive aggregation cut before admission because one chunk's drain was estimated over the pressure part bound.", ValueType::Number) \
    M(AdaptiveAggregationStagedChunkPiecesOverBound, "How many pieces of a cut staged chunk the adaptive aggregation prepared still over the bound the chunk was cut to meet. A chunk is cut along bucket boundaries, and record by record inside a bucket that is over the bound on its own, so this is a single record whose staged bytes alone are over the bound; such a piece is claimed whole, and its drain can overshoot the pressure part by that much.", ValueType::Number) \
    ...
    M(AdaptiveAggregationSealNormalizations, "How many staged argument columns adaptive conversion normalized from a wrapped representation (Const, Replicated, Sparse, LowCardinality) to the dense form the drain consumes.", ValueType::Number) \
